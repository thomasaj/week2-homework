...
wea_data = JSON.parse(open("http://api.openweathermap.org/data/2.5/weather?lat=#{loc_data["results"][0]["geometry"]["location"]["lat"]}&lon=#{loc_data["results"][0]["geometry"]["location"]["lng"]}").read)
...
temp = '%.1f' % ( ( wea_data["main"]["temp"].to_f - 273.15 ) * 9/5 + 32)
puts "#{temp}f"
